...
// If you are requesting another user's liked pins 
...
try {
    ...
    $sql =  'SELECT  
            (SELECT COUNT(*) FROM tLiked INNER JOIN tPin ON tLiked.nPinID = tPin.nPinID WHERE tLiked.nUserID = :user_ID AND tLiked.bActive = 1 AND tPin.bActive = 1) AS limit_pins,
            tPin.nPinID, 
            tPin.cFileName, 
            tPin.cFileExtension, 
            tPin.cDescription, 
            ...
            WHERE tUser.bActive = 1 AND creator.bActive = 1 AND tUser.nUserID = :user_ID AND tPin.bActive = 1 AND tLiked.bActive = 1
    ...
    if( ! $result ){
        _res(200, ['info'=> isset($_GET['user_ID']) ? 'There aren’t any pins on the liked board yet' : 'There aren’t any pins on your liked board yet', 'error'=>__LINE__, 'data'=> []]);
    }
